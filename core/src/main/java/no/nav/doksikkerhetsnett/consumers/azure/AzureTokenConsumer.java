...
import static no.nav.doksikkerhetsnett.config.cache.LokalCacheConfig.AZURE_CACHE;
...
public class AzureTokenConsumer {
	...
	public AzureTokenConsumer(AzureProperties azureProperties,
		...
		final CloseableHttpClient httpClient = createHttpClient(dokSikkerhetsnettProperties.getProxy(), httpClientConnectionManager);
	...
	private CloseableHttpClient createHttpClient(DokSikkerhetsnettProperties.Proxy proxy,
												 HttpClientConnectionManager httpClientConnectionManager) {
		if (proxy.isSet()) {
